...
deploy:
    helm install patcher charts/patcher-chart --wait --create-namespace --namespace patcher --set image.tag=latest --set replicaCount=2 -f deploy/debug-patcher-values.yaml

debug:
    kubectl apply -f deploy/debug.yaml
    kubectl create namespace ignored
    kubectl apply --namespace=ignored -f deploy/debug.yaml
...
delete:
    ...
    kubectl delete --namespace=ignored -f deploy/debug.yaml || /bin/true
    kubectl delete namespace ignored